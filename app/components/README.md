...
The `app/components` directory contains all code specific to this application. Apart
...
within the `app/components` directory that conceptually corresponds to the top-level
...
As `DistrictEuro` is quite minimal, take a look at the two provided submodules
...
requiring all the modules from `app/components` that we need. We must load these now to
...
However, the modules from `app/shared` should be required by the app
...
One of the design philosophies of `DistrictEuro` is that tests should exist
